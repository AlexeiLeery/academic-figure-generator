...
export function AdminUsers() {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setCreateForm({ email: '', password: '', display_name: '', is_admin: false, nanobanana_images_quota: 0 });
            await fetchUsers();
        } catch (e: any) {
    ...
    const handleCreditAdjust = async () => {
        ...
        if (isNaN(delta) || delta === 0) {
            alert('请输入有效的额度调整值（正数增加，负数减少）');
            return;
        }
        setIsAdjustingCredit(true);
        try {
            await api.post(`/admin/users/${creditTarget.id}/credits`, { delta });
            setShowCreditDialog(false);
            await fetchUsers();
        } catch (e: any) {
            alert(e.response?.data?.detail || '调整额度失败');
        } finally {
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead className="text-center">生图额度</TableHead>
                                ...
                                users.map((u) => (
                                    <TableRow key={u.id}>
                                        ...
                                        <TableCell className="text-center">
                                            <span className={`font-mono font-bold text-lg ${u.nanobanana_images_quota > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                                                {u.nanobanana_images_quota}
                                            </span>
                                        </TableCell>
            ...
            <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
                <DialogContent>
                    ...
                    <div className="space-y-4 py-2">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="create_quota">初始生图额度</Label>
                            <Input
                                id="create_quota"
                                type="number"
                                min={0}
                                placeholder="0"
                                value={createForm.nanobanana_images_quota}
                                onChange={(e) => setCreateForm({ ...createForm, nanobanana_images_quota: parseInt(e.target.value) || 0 })}
                            />
                        </div>
            ...
            <Dialog open={showCreditDialog} onOpenChange={setShowCreditDialog}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>调整生图额度</DialogTitle>
                        <DialogDescription>
                    ...
                    <div className="space-y-4 py-2">
                        <div className="text-center p-4 rounded-lg bg-muted/50">
                            <div className="text-sm text-muted-foreground">当前额度</div>
                            <div className={`text-4xl font-bold mt-1 ${(creditTarget?.nanobanana_images_quota ?? 0) > 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                                {creditTarget?.nanobanana_images_quota ?? 0}
                            </div>
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="credit_delta">调整数量</Label>
                            <Input
                                id="credit_delta"
                                type="number"
                            ...
                            <p className="text-xs text-muted-foreground">
                                输入正数增加额度，输入负数减少额度。每次生图消耗 1 额度。
                            </p>
                        </div>
                        {creditDelta && !isNaN(parseInt(creditDelta)) && parseInt(creditDelta) !== 0 && (
                            <div className="text-center p-3 rounded-lg border">
                                <span className="text-sm text-muted-foreground">调整后额度：</span>
                                <span className="text-lg font-bold ml-2">
                                    {(creditTarget?.nanobanana_images_quota ?? 0) + parseInt(creditDelta)}
                                </span>
                            </div>
                        )}
                    </div>
